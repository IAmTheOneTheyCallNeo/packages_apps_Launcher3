...
public class QsbContainerView extends Qsb {
    private static final Rect sTempRect = new Rect();
    private final TransformingTouchDelegate mTouchDelegate;

    public QsbContainerView(Context paramContext) {
        this(paramContext, null);
    }

    public QsbContainerView(Context paramContext, AttributeSet paramAttributeSet) {
        this(paramContext, paramAttributeSet, 0);
    }

    public QsbContainerView(Context paramContext, AttributeSet paramAttributeSet, int paramInt) {
    ...
    protected void onAttachedToWindow() {
        ...
        if (mTouchDelegate != null) {
            mLauncher.getWorkspace().findViewById(
                    R.id.workspace_blocked_row).setTouchDelegate(mTouchDelegate);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (deviceProfile.isVerticalBarLayout()) {
            final int columnsWidth = qsbOverlapMargin + getResources().getDimensionPixelSize(
                    R.dimen.qsb_button_elevation);
            n6 = size;
            marginStart = columnsWidth;
        } else {
            final int n7 = size - workspacePadding.left - workspacePadding.right;
            n6 = DeviceProfile.calculateCellWidth(n7,
                    deviceProfile.inv.numColumns) * deviceProfile.inv.numColumns;
    ...
    protected void onLayout(boolean z, int i, int i2, int i3, int i4) {
        ...
        if (mTouchDelegate != null) {
            ...
            if (Utilities.isRtl(getResources())) {
                i5 = mQsbView.getLeft() - mLauncher.getDeviceProfile().getWorkspacePadding(
                        sTempRect).left;
            }
            mTouchDelegate.setBounds(i5, mQsbView.getTop(), mQsbView.getWidth() + i5,
                    mQsbView.getBottom());
        }
    }
