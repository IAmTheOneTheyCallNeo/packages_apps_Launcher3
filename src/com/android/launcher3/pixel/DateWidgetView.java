...
public class DateWidgetView extends LinearLayout implements TextWatcher {
    ...
    private void init() {
        ...
        if (locale != null && Locale.ENGLISH.getLanguage().equals(locale.getLanguage())) {
            ...
            dateText2.setPadding(0, 0, 0, ((int) (Math.abs(
                    paint.getFontMetrics().ascent) - ((float) height))) / 2);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        width = (deviceProfile.inv.numColumns - Math.max(1, (int) Math.ceil((double) (
                getResources().getDimension(
                R.dimen.qsb_min_width_with_mic) / ((float) size))))) * size;
        text = "";
        update();
